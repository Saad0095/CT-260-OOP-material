...
template <typename T> void updateSalary(Employee<T>& e, T s);
...
template <class T>
bool compareSalary(const Employee<T>& a, const Employee<T>& b) {
...
class Payroll{
	...
	public:
		...
		void sortBySalary() {
    		sort(stored.begin(), stored.end(), compareSalary<T>);
		}
		void displayStore(){
		...
		Employee<T>& getEmp(size_t index){
